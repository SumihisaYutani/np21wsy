...
// �v���C�}���T�[�t�F�C�X��DirectDraw DDBLT_COLORFILL�ŗ�O���o���������悤�Ȃ̂ő��
	...
	if (scrnmode & SCRNMODE_FULLSCREEN) {
		...
		if(lastscrnmode & SCRNMODE_FULLSCREEN){
			...
			if(PathFileExistsA(szModulePath) && !PathIsDirectoryA(szModulePath)){
				// DXGL���g���Ă������Ȃ̂őf���ȃR�[�h�ɕύX
				SetWindowLong(g_hWndMain, GWL_STYLE, winstyle);
				SetWindowLong(g_hWndMain, GWL_EXSTYLE, winstyleex);
			}else{
				ShowWindow(g_hWndMain, SW_HIDE); // Aero�Ȋ��Ńt���X�N���[�����E�B���h�E�̎��ɃV�X�e���A�C�R����������΍�̂��߂̂��܂��Ȃ��i����1�j
				SetWindowLong(g_hWndMain, GWL_STYLE, winstyle);
				SetWindowLong(g_hWndMain, GWL_EXSTYLE, winstyleex);
				ShowWindow(g_hWndMain, SW_SHOWNORMAL); // Aero�Ȋ���(ry�i����2�j
				ShowWindow(g_hWndMain, SW_HIDE); // Aero�Ȋ���(ry�i����3�j
				ShowWindow(g_hWndMain, SW_SHOWNORMAL); // Aero�Ȋ���(ry�i����4�j
			}
...
void scrnmngDD_updatefsres(void) {
	...
	if(np2wab.lastWidth!=width || np2wab.lastHeight!=height){
		...
		if((g_scrnmode & SCRNMODE_FULLSCREEN)!=0){
			...
			scrnmngDD_destroy();
			if (scrnmngDD_create(g_scrnmode) != SUCCESS) {
				if (scrnmngDD_create(g_scrnmode | SCRNMODE_FULLSCREEN) != SUCCESS) { // �t���X�N���[���Ń��g���C
					scrnmng_create_pending = true;
					//PostQuitMessage(0);
					dd_leave_criticalsection();
					return;
