...
BRESULT scrnmngD3D_check() {
	// Direct3D���g�p�ł��邩�`�F�b�N + �x���_�m�F
	HMODULE hModule;
...
BRESULT scrnmngD3D_create(UINT8 scrnmode) {
	...
	np2oscfg.d3d_exclusive = 0;// �r�����[�h�͔p�~
	...
	if (scrnmode & SCRNMODE_FULLSCREEN) {
		...
		if(!np2oscfg.d3d_exclusive){
			...
			fscrnmod |= FSCRNMOD_SAMERES | FSCRNMOD_SAMEBPP; // �r�����[�h���g��Ȃ��Ƃ��A�𑜓x�ύX���Ȃ����Ƃɂ���i�ł��Ȃ��͂Ȃ����ʓ|�Ȃ��Ƃ��̏�Ȃ��j
			GetWindowRect(g_hWndMain, &mondata.np2windowrect); // �L�E�B���h�E�̈ʒu�ƃT�C�Y���擾
			EnumDisplayMonitors(NULL, NULL, (MONITORENUMPROC)monitorInfoEnumProc, (LPARAM)&mondata); // �L�̂��郂�j�^�[��T��
			if(!mondata.found){
				// �L�ǂ��ɂ����Ȃ����ɂ̓v���C�}�����j�^��
			...
			MoveWindow(g_hWndMain, mondata.monitorrect.left, mondata.monitorrect.top, 
				mondata.monitorrect.right - mondata.monitorrect.left, mondata.monitorrect.bottom - mondata.monitorrect.top, TRUE); // �E�B���h�E�T�C�Y��S��ʂɕς���
		}
		if(np2_multithread_Enabled()){
			fscrnmod |= FSCRNMOD_SAMERES | FSCRNMOD_SAMEBPP; // �}���`�X���b�h���[�h�̂Ƃ����𑜓x�ύX���Ȃ����Ƃɂ���i�蔲���j
		}
	...
	d3d.cliping = 0;
	//if (scrnmode & SCRNMODE_FULLSCREEN) {
		renewalclientsize(TRUE); // XXX: �X�i�b�v���������N����̂Ŏb��TRUE
	//}
	lastscrnmode = scrnmode;
//	screenupdate = 3;					// update!
#if defined(SUPPORT_WAB)
	mt_wabpausedrawing = 0; // MultiThread�΍�
#endif
...
void scrnmngD3D_updatefsres(void) {
	...
	if(np2wab.lastWidth!=width || np2wab.lastHeight!=height){
		...
		if((g_scrnmode & SCRNMODE_FULLSCREEN)!=0){
			...
			scrnmngD3D_destroy();
			if (scrnmngD3D_create(g_scrnmode) != SUCCESS) {
				//if (scrnmngD3D_create(g_scrnmode | SCRNMODE_FULLSCREEN) != SUCCESS) { // �t���X�N���[���Ń��g���C
					//PostQuitMessage(0);
					scrnmng_create_pending = true;
					d3d_leave_criticalsection();
					return;
...
// �E�B���h�E�A�N�Z�����[�^��ʓ]�� GDI Device Independent Bitmap -> Direct3D WAB surface
...
// �E�B���h�E�A�N�Z�����[�^��ʓ]�� Direct3D WAB surface -> Direct3D back surface
