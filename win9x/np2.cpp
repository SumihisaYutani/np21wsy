...
#endif
extern bool scrnmng_create_pending; // �O���t�B�b�N�����_�������ۗ���
...
static	TCHAR		szClassName[] = _T("NP2-MainWindow");
		...
		NP2OSCFG	np2oscfg = {
						...
						CSoundMng::kDSound3, {0},

#if defined(SUPPORT_VSTi)
						{'%','P','r','o','g','r','a','m','F','i','l','e','s','%','\\','R','o','l','a','n','d','\\','S','o','u','n','d',' ','C','a','n','v','a','s',' ','V','A','\\','S','O','U','N','D',' ','C','a','n','v','a','s',' ','V','A','.','d','l','l',0},
#endif	// defined(SUPPORT_VSTi)
						0, 
...
static  UINT		lateframecount; // �t���[���x�ꐔ
...
static const OEMCHAR np2flagext[] = OEMTEXT("S%03d");
#if defined(_WIN64)
...
// ASCII -> 98�L�[�R�[�h�\(np21w ver0.86 rev22)
char vkeylist[256] = {0};
char shift_on[256] = {0};

// �}���`�X���b�h�p
#if defined(SUPPORT_MULTITHREAD)
static int np2_multithread_requestswitch = 0; // �}���`�X���b�h���[�h�ؑ֗v���t���O
static int np2_multithread_enable = 0; // �}���`�X���b�h���[�h�L���t���O
static BOOL np2_multithread_initialized = 0; // �}���`�X���b�h���[�h�������ς݃t���O
static HANDLE	np2_multithread_hThread = NULL; // �G�~�����[�V�����p�X���b�h
static CRITICAL_SECTION	np2_multithread_hThread_cs = {0}; // �G�~�����[�V�����p�X���b�h�@�N���e�B�J���Z�N�V����
static BOOL	np2_multithread_hThread_requestexit = FALSE; // �G�~�����[�V�����p�X���b�h�@�I���v���t���O
static bool np2_multithread_pauseemulation = false; // �G�~�����[�V�����ꎞ��~�t���O
static bool np2_multithread_pausing = false; // �G�~�����[�V�����ꎞ��~���t���O
...
// �V�X�e���L�[�t�b�N�p
#ifdef HOOK_SYSKEY
static HANDLE	np2_hThreadKeyHook = NULL; // �L�[�t�b�N�p�X���b�h
static int		np2_hThreadKeyHookexit = 0; // �X���b�h�I���t���O
...
static unsigned int __stdcall np2_ThreadFuncKeyHook(LPVOID vdParam) 
{
	...
	ShowWindow( np2_hThreadKeyHookhWnd, SW_HIDE ); // �O�̂���
...
// �^�C�g���o�[�̉��ʁE�}�E�X���x ������\���p
#define TMRSYSMNG_ID	9898 // ���Ɣ��Ȃ��悤�ɂ��邱��
...
WINLOCEX np2_winlocexallwin(HWND base) {
	...
	if (base != g_hWndMain) {		// hWndMain�̂ݑS�̈ړ�
		base = NULL;
	}
	...
	if (renewal) {
		...
		if (scrnmng_create(newmode) == SUCCESS) {
			...
			if(np2oscfg.scrnmode != g_scrnmode){
				np2oscfg.scrnmode = g_scrnmode; // Screen��ԕۑ�
...
static void np2updatemenu() {
	static char drvMenuVisible[4][26] = {0}; // ���h���C�u���j���[�̕\�����
	char drvAvailable[26] = {0}; // �g������h���C�u
		...
		case IDM_COPYPASTE_COPYTVRAM:
			{
				HGLOBAL hMem;
				OEMCHAR *lpMem;
				hMem = GlobalAlloc(GHND,0x4000); // �A�g���r���[�g���������Ȃ�̂�0x4000�ŏ\��
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	switch (msg) {
		//	�C���[�W�t�@�C���̂c���c�ɑΉ�(Kai1)
		case WM_DROPFILES:
			np2_multithread_EnterCriticalSection();
			if(np2oscfg.dragdrop){
				int		files;				//	Kai1�ǉ�
				OEMCHAR	fname[MAX_PATH];	//	Kai1�ǉ�
				const OEMCHAR	*ext;		//	Kai1�ǉ�
		...
		case WM_SYSCOMMAND:
			...
			switch(wParam) {
				...
				case IDM_STATEMANAGER:
					{
						CStateManagerWnd* pStateManager = CStateManagerWnd::GetInstance();
						if (!pStateManager) {
							CStateManagerWnd::CreateInstance();
							pStateManager = CStateManagerWnd::GetInstance();
						}
						if (pStateManager) {
							pStateManager->Show(TRUE);
						}
					}
					break;
...
//				scrnmng_update();
		...
		case WM_KEYDOWN:
			autokey_sendbufferpos = autokey_sendbufferlen; // �R�s�y�����I�� np21w ver0.86 rev22
			if (wParam == VK_F11) {
		...
		case WM_MOUSEMOVE:
			...
			if(np2oscfg.mouse_nc){
				...
				int xPos, yPos;
				int mouseon = 1;
				static int mousebufX = 0; // �}�E�X�ړ��o�b�t�@(X)
				static int mousebufY = 0; // �}�E�X�ړ��o�b�t�@(Y)
...
#define MOUSE_EDGE_ACM	4
		...
		case WM_LBUTTONDOWN:
			...
			// Mouse capture temporarily disabled for build
		...
		case WM_LBUTTONUP:
			...
			// Mouse capture release temporarily disabled for build
			// if (mousecapturemode)
			// {
			//	ReleaseCapture();
			//	mousecapturemode = 0;
...
void loadNP2INI(const OEMCHAR *fname){
	// Debug: loadNP2INI function entry logging
	{
		TCHAR debugMsg[512];
		OutputDebugString(_T("=== loadNP2INI Function Starting ==="));
		_stprintf_s(debugMsg, _countof(debugMsg), _T("loadNP2INI called with fname: %s"), fname ? fname : _T("(null)"));
	...
	if(np2oscfg.dragdrop)
		DragAcceptFiles(hWnd, TRUE);	//	�C���[�W�t�@�C���̂c���c�ɑΉ�(Kai1)
	else
	...
	//sysmenu_initialize(hSysMenu); // �Ή��ʓ|������
	
	HMENU hMenu = np2class_gethmenu(hWnd);
	//xmenu_initialize(hMenu); // �Ή��ʓ|������
	xmenu_update(hMenu);
	...
	// Debug: Sound system initialization logging
	{
		TCHAR debugMsg[512];
		OutputDebugString(_T("=== Sound System Initialization Starting ==="));

		__try {
			OutputDebugString(_T("Calling CSoundMng::Initialize()"));
			CSoundMng::Initialize();
			OutputDebugString(_T("CSoundMng::Initialize() completed successfully"));

			OutputDebugString(_T("Calling OpenSoundDevice()"));
			OpenSoundDevice(hWnd);
			OutputDebugString(_T("OpenSoundDevice() completed"));

			_stprintf_s(debugMsg, _countof(debugMsg), _T("Attempting to open sound device - Type: %d, Name: %s"),
				static_cast<int>(np2oscfg.cSoundDeviceType),
				np2oscfg.szSoundDeviceName[0] ? np2oscfg.szSoundDeviceName : _T("(default)"));
			OutputDebugString(debugMsg);

			if (CSoundMng::GetInstance()->Open(static_cast<CSoundMng::DeviceType>(np2oscfg.cSoundDeviceType), np2oscfg.szSoundDeviceName, hWnd))
			{
				OutputDebugString(_T("Sound device opened successfully, loading PCM resources"));
				...
				OutputDebugString(_T("PCM resources loaded successfully"));
			}
			else {
				OutputDebugString(_T("WARNING: Failed to open sound device"));
			}
			OutputDebugString(_T("=== Sound System Initialization Completed ==="));
		}
		__except(EXCEPTION_EXECUTE_HANDLER) {
			_stprintf_s(debugMsg, _countof(debugMsg), _T("EXCEPTION in Sound initialization: 0x%08X"), GetExceptionCode());
			OutputDebugString(debugMsg);
			OutputDebugString(_T("=== Sound System Initialization CRASHED ==="));
			// Continue execution - sound failure shouldn't be fatal
		}
	}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst,
	...
	// Debug: WinMain function entry logging
	{
		TCHAR debugMsg[512];
		OutputDebugString(_T("=== NP21WSY WinMain Starting ==="));
		_stprintf_s(debugMsg, _countof(debugMsg), _T("WinMain called with nCmdShow: %d, lpszCmdLine: %hs"), nCmdShow, lpszCmdLine ? lpszCmdLine : "(null)");
		OutputDebugString(debugMsg);
		OutputDebugString(_T("Initializing variables and debug flags"));
	}
...
#ifdef _DEBUG
	// �g���Ƃ���stdlib.h��crtdbg.h���C���N���[�h����
	OutputDebugString(_T("Setting up debug memory leak detection"));
...
#define WINVER2 0x0500
	...
	OutputDebugString(_T("Initializing memory system"));
	_MEM_INIT();

	OutputDebugString(_T("Initializing CWndProc"));
	CWndProc::Initialize(hInstance);

	OutputDebugString(_T("Initializing CSubWndBase"));
	CSubWndBase::Initialize(hInstance);

#if defined(SUPPORT_VSTi)
	OutputDebugString(_T("Initializing CVstEditWnd (VST support)"));
	CVstEditWnd::Initialize(hInstance);
#endif	// defined(SUPPORT_VSTi)

	OutputDebugString(_T("Getting module filename"));
	...
	g_hWndMain = hWnd;
	FILE* fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "g_hWndMain set to: %p\n", g_hWndMain);
		fclose(fp);
	}
	winloc_DisableCornerRound(g_hWndMain);
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "winloc_DisableCornerRound completed\n");
		fclose(fp);
	}
	...
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Calling mousemng_initialize()\n");
		fclose(fp);
	}
	mousemng_initialize(); // �ꏊ�ړ� np21w ver0.96 rev13
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "mousemng_initialize() completed\n");
		fclose(fp);
	}

	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Calling scrnmng_initialize()\n");
		fclose(fp);
	}
	scrnmng_initialize();
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "scrnmng_initialize() completed\n");
		fclose(fp);
	}

	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Checking dragdrop: %d\n", np2oscfg.dragdrop);
		fclose(fp);
	}
	if(np2oscfg.dragdrop)
		DragAcceptFiles(hWnd, TRUE);	//	�C���[�W�t�@�C���̂c���c�ɑΉ�(Kai1)
	...
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Calling ShowWindow\n");
		fclose(fp);
	}
	ShowWindow(hWnd, nCmdShow);
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "ShowWindow completed\n");
		fclose(fp);
	}

	// Auto-start State Manager window
	...
	if (!stateManagerInitialized) {
		...
		if (pStateManager && pStateManager->Create()) {
			pStateManager->Show(TRUE);
		}
	...
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "UpdateWindow completed\n");
		fclose(fp);
	}
	
	SetWindowPos(hWnd, NULL, winx, winy, 0, 0, SWP_NOSIZE|SWP_NOZORDER|SWP_NOZORDER|SWP_NOACTIVATE|SWP_FRAMECHANGED); // Win10���ŃE�B���h�E�ʒu���������̑΍�
	...
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Before sysmenu_initialize\n");
		fclose(fp);
	}
	sysmenu_initialize(GetSystemMenu(hWnd, FALSE));
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "After sysmenu_initialize\n");
		fclose(fp);
	}

	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Getting hMenu\n");
		fclose(fp);
	}
	HMENU hMenu = np2class_gethmenu(hWnd);
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "hMenu obtained: %p\n", hMenu);
		fclose(fp);
	}
	xmenu_initialize(hMenu);
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "xmenu_initialize completed\n");
		fclose(fp);
	}
	xmenu_update(hMenu);
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "xmenu_update completed\n");
		fclose(fp);
	}
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Checking np2help file\n");
		fclose(fp);
	}
	if (file_attr_c(np2help) == -1)								// ver0.30
	{
	...
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Calling DrawMenuBar\n");
		fclose(fp);
	}
	DrawMenuBar(hWnd);
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "DrawMenuBar completed\n");
		fclose(fp);
	}

	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Setting screen mode\n");
		fclose(fp);
	}
	if(np2oscfg.savescrn){
		g_scrnmode = np2oscfg.scrnmode;
	...
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Screen mode set: 0x%X\n", g_scrnmode);
		fclose(fp);
	}
	// Debug: DirectDraw initialization logging with error details
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "Starting DirectDraw initialization\n");
		fclose(fp);
	}
	TCHAR debugMsg[512];
	OutputDebugString(_T("=== DirectDraw Initialization Starting ==="));

	_stprintf_s(debugMsg, _countof(debugMsg), _T("Initial screen mode: 0x%08X"), g_scrnmode);
	OutputDebugString(debugMsg);

	_stprintf_s(debugMsg, _countof(debugMsg), _T("SCRNMODE_FULLSCREEN=%d, SCRNMODE_HIGHCOLOR=%d"),
		(g_scrnmode & SCRNMODE_FULLSCREEN) ? 1 : 0,
		(g_scrnmode & SCRNMODE_HIGHCOLOR) ? 1 : 0);
	OutputDebugString(debugMsg);
	fp = fopen("debug_direct.log", "a");
	if (fp) {
		fprintf(fp, "About to call scrnmng_create\n");
		fclose(fp);
	}

	// Add try-catch equivalent error handling
	__try {
		OutputDebugString(_T("Calling scrnmng_create (first attempt)"));
		fp = fopen("debug_direct.log", "a");
		if (fp) {
			fprintf(fp, "Calling scrnmng_create with mode: 0x%X\n", g_scrnmode);
			fclose(fp);
		}
		int result = scrnmng_create(g_scrnmode);
		fp = fopen("debug_direct.log", "a");
		if (fp) {
			fprintf(fp, "scrnmng_create returned: %d\n", result);
			fclose(fp);
		}

		if (result != SUCCESS) {
			_stprintf_s(debugMsg, _countof(debugMsg), _T("scrnmng_create FAILED (result=%d), trying windowed mode"), result);
			OutputDebugString(debugMsg);

			g_scrnmode ^= SCRNMODE_FULLSCREEN;
			_stprintf_s(debugMsg, _countof(debugMsg), _T("Retrying with windowed mode: 0x%08X"), g_scrnmode);
			OutputDebugString(debugMsg);

			result = scrnmng_create(g_scrnmode);
			if (result != SUCCESS) {
				_stprintf_s(debugMsg, _countof(debugMsg), _T("scrnmng_create FAILED completely (result=%d)"), result);
				OutputDebugString(debugMsg);
				OutputDebugString(_T("=== DirectDraw Initialization FAILED ==="));
				messagebox(hWnd, MAKEINTRESOURCE(IDS_ERROR_DIRECTDRAW), MB_OK | MB_ICONSTOP);
				UnloadExternalResource();
				TRACETERM();
				dosio_term();
			...
			OutputDebugString(_T("scrnmng_create succeeded in windowed mode"));
		} else {
			OutputDebugString(_T("scrnmng_create succeeded on first attempt"));
			fp = fopen("debug_direct.log", "a");
			if (fp) {
				fprintf(fp, "scrnmng_create succeeded on first attempt\n");
				fclose(fp);
			}
		}
		OutputDebugString(_T("=== DirectDraw Initialization SUCCESS ==="));
		fp = fopen("debug_direct.log", "a");
		if (fp) {
			fprintf(fp, "DirectDraw initialization completed\n");
			fclose(fp);
		}

	}
	__except(EXCEPTION_EXECUTE_HANDLER) {
		_stprintf_s(debugMsg, _countof(debugMsg), _T("EXCEPTION in scrnmng_create: 0x%08X"), GetExceptionCode());
		OutputDebugString(debugMsg);
		OutputDebugString(_T("=== DirectDraw Initialization CRASHED ==="));
		messagebox(hWnd, _T("DirectDraw initialization crashed"), MB_OK | MB_ICONSTOP);
		UnloadExternalResource();
		TRACETERM();
		dosio_term();
		return(FALSE);
